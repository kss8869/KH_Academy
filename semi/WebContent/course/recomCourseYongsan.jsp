...
<%@ page import="course.*" %>
<jsp:useBean id="yfdao" class="course.YongsanFoodDAO" scope="page"/>
<jsp:useBean id="ypdao" class="course.YongsanPlaceDAO" scope="page"/>
...
int totalCnt=yfdao.getTotalCnt();//총 게시물 수 
int totalCnt2=ypdao.getTotalCnt();
		...
		<table>
			...
			<tfoot>
				<tr>
					...
					if(userGroup!=0	){
						%>
						<a href="recomCourseYongsan.jsp?cp=<%=(userGroup-1)*pageSize+pageSize%>">&lt;&lt;</a>
						<%
					}
					for(int i=userGroup*pageSize+1;i<=userGroup*pageSize+pageSize;i++){
						%>
						&nbsp;&nbsp;<a href="recomCourseYongsan.jsp?cp=<%=i%>&cp2=<%=cp2 %>"><%=i %></a>&nbsp;&nbsp;
						<%
						if(i==totalPage)break;
					}
					if(userGroup!=((totalPage/pageSize)-(totalPage%pageSize==0?1:0))){
						%>
						<a href="recomCourseYongsan.jsp?cp=<%=((userGroup+1)*pageSize+1)%>">&gt;&gt;</a>
						<%
					}
					%>
					</td>
			...
			<tbody>
			<%
			ArrayList<YongsanFoodDTO> arr=yfdao.courseList(cp, listSize);
			if(arr==null||arr.size()==0){
				...
				for(int i=0;i<arr.size();i++){
					...
					<tr>
						<td>
						<a href="recomFoodDetailYongsan.jsp?location=<%=location %>&idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>&cp2=<%=cp2%>">
						<img src="/semi/img/<%=localname %>/<%=arr2.get(0).getFilename()%>" width="200">
						</a>
						</td>
					</tr>
					<tr>
						<td><a href="recomFoodDetailYongsan.jsp?location=<%=location %>&idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>&cp2=<%=cp2%>"><%=arr.get(i).getRname() %></a></td>
					</tr>
					<tr>
		...
		<table>
			...
			<tfoot>
				<tr>
					...
					if(userGroup2!=0){
						%>
						<a href="recomCourseYongsan.jsp?cp2=<%=(userGroup2-1)*pageSize+pageSize%>">&lt;&lt;</a>
						<%
					}
					for(int i=userGroup2*pageSize+1;i<=userGroup2*pageSize+pageSize;i++){
						%>
						&nbsp;&nbsp;<a href="recomCourseYongsan.jsp?cp=<%=cp %>&cp2=<%=i%>"><%=i %></a>&nbsp;&nbsp;
						<%
						if(i==totalPage2)break;
					}
					if(userGroup2!=((totalPage2/pageSize)-(totalPage2%pageSize==0?1:0))){
						%>
						<a href="recomCourseYongsan.jsp?cp2=<%=((userGroup2+1)*pageSize+1)%>">&gt;&gt;</a>
						<%
					}
					%>
					</td>
			...
			<tbody>
			<%
			ArrayList<YongsanPlaceDTO> arr2=ypdao.courseList(cp2, listSize);
			if(arr==null||arr.size()==0){
				%>
				<tr>
					<td colspan="4" align="center">등록된 게시글이 없습니다.</td>
				...
				for(int i=0;i<arr2.size();i++){
					...
					<tr>
						<td>
						<a href="recomCourseDetailYongsan.jsp?location=<%=location %>&idx=<%=arr2.get(i).getIdx()%>&cp=<%=cp %>&cp2=<%=cp2%>">
						<img src="/semi/img/<%=localname %>/<%=arr3.get(0).getFilename()%>" width="200">
						</a>
						</td>
					</tr>
					<tr>
						<td><a href="recomCourseDetailYongsan.jsp?location=<%=location %>&idx=<%=arr2.get(i).getIdx()%>&cp=<%=cp %>&cp2=<%=cp2%>"><%=arr2.get(i).getPlace() %></a></td>
					</tr>
					<tr>
