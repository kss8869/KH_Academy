...
<%@ page import="course.*" %>
<jsp:useBean id="dfdao" class="course.DongdaemoonFoodDAO" scope="page"/>
<jsp:useBean id="dpdao" class="course.DongdaemoonPlaceDAO" scope="page"/>
...
int totalCnt=dfdao.getTotalCnt();//총 게시물 수 
int totalCnt2=dpdao.getTotalCnt();
		...
		<table>
			...
			<tfoot>
				<tr>
					...
					if(userGroup!=0	){
						%>
						<a href="recomCourseDongdaemoon.jsp?cp=<%=(userGroup-1)*pageSize+pageSize%>">&lt;&lt;</a>
						<%
					}
					for(int i=userGroup*pageSize+1;i<=userGroup*pageSize+pageSize;i++){
						%>
						&nbsp;&nbsp;<a href="recomCourseDongdaemoon.jsp?cp=<%=i%>&cp2=<%=cp2 %>"><%=i %></a>&nbsp;&nbsp;
						<%
						if(i==totalPage)break;
					}
					if(userGroup!=((totalPage/pageSize)-(totalPage%pageSize==0?1:0))){
						%>
						<a href="recomCourseDongdaemoon.jsp?cp=<%=((userGroup+1)*pageSize+1)%>">&gt;&gt;</a>
						<%
					}
					%>
					</td>
			...
			<tbody>
			<%
			ArrayList<DongdaemoonFoodDTO> arr=dfdao.courseList(cp, listSize);
			if(arr==null||arr.size()==0){
				...
				for(int i=0;i<arr.size();i++){
					...
					if(arr2==null||arr2.size()==0){
						...
						<tr>
							<td>
								<a href="recomFoodDetailDongdaemoon.jsp?location=<%=location %>&idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>&cp2=<%=cp2%>">
					...
					%>
					<tr>
						<td><a href="recomFoodDetailDongdaemoon.jsp?location=<%=location %>&idx=<%=arr.get(i).getIdx()%>&cp=<%=cp%>&cp2=<%=cp2%>"><%=arr.get(i).getRname() %></a></td>
					</tr>
					<tr>
		...
		<table>
			...
			<tfoot>
				<tr>
					...
					if(userGroup2!=0){
						%>
						<a href="recomCourseDongdaemoon.jsp?cp2=<%=(userGroup2-1)*pageSize+pageSize%>">&lt;&lt;</a>
						<%
					}
					for(int i=userGroup2*pageSize+1;i<=userGroup2*pageSize+pageSize;i++){
						%>
						&nbsp;&nbsp;<a href="recomCourseDongdaemoon.jsp?cp=<%=cp %>&cp2=<%=i%>"><%=i %></a>&nbsp;&nbsp;
						<%
						if(i==totalPage2)break;
					}
					if(userGroup2!=((totalPage2/pageSize)-(totalPage2%pageSize==0?1:0))){
						%>
						<a href="recomCourseDongdaemoon.jsp?cp2=<%=((userGroup2+1)*pageSize+1)%>">&gt;&gt;</a>
						<%
					}
					%>
					</td>
			...
			<tbody>
			<%
			ArrayList<DongdaemoonPlaceDTO> arr2=dpdao.courseList(cp2, listSize);
			if(arr==null||arr2.size()==0){
				...
				for(int i=0;i<arr2.size();i++){
					...
					if(arr3==null||arr3.size()==0){
						...
						<tr>
							<td>
								<a href="recomCourseDetailDongdaemoon.jsp?location=<%=location %>&idx=<%=arr2.get(i).getIdx()%>&cp=<%=cp %>&cp2=<%=cp2%>">
					...
					%>
					<tr>
						<td><a href="recomCourseDetailDongdaemoon.jsp?location=<%=location %>&idx=<%=arr2.get(i).getIdx()%>&cp=<%=cp %>&cp2=<%=cp2%>"><%=arr2.get(i).getPlace() %></a></td>
					</tr>
					<tr>
